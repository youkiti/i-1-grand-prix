...
- **最重要フィルタ**: 「{{ focus }}」に直接関係のない内容は**すべて除外**してください。以下は除外対象の例です：
  - 事務的な詳細（会議の運営方法、委員の選任、リモート開催ルール、議事録の公開方法など）
  - **別テーマの政策議論**（ドキュメント内で言及されていても、「{{ focus }}」と直接関係しない税制、社会保障、エネルギー政策、復興政策などの論点）
  - 「{{ focus }}」の前提や背景として軽く触れられているだけの論点（深く議論されていないもの）
- **判断基準**: その論点が「{{ focus }}」の実現・決定・設計に**直接影響を与えるか**を基準に判断してください。

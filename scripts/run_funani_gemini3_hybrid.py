"""
Funani パイプライン一括実行スクリプト (Gemini 3 版)
- Stage 1-2: gemini-3-flash-preview
...
def main():
    print(f"\n{'#'*60}")
    print("# Funani Pipeline: Gemini 3 Hybrid")
    print("# Stage 1-2: gemini-3-flash-preview")
    print("# Stage 3: gemini-3-pro-preview")
    print(f"{'#'*60}")
    ...
    run_command([
        ...
        "--focus", FOCUS,
        "--log-dir", LOG_DIR
    ], f"Stage 1: pre_hypothesis_iterative ({FLASH_MODEL})")
    ...
    run_command([
        ...
        "--focus", FOCUS,
        "--log-dir", LOG_DIR
    ], f"Stage 2: pubcom_aggregate ({FLASH_MODEL})")
